...
<?php 
 $menus= $this->db->where('parentId', 0)->order_by('id', 'ASC')->get('menu')->result();    
 foreach ($menus as $key => $value) {
     $menus[$key]->submenu = $this->db->where('parentId', $value->id)->order_by('id', 'ASC')->get('menu')->result();
 }
?>
                    ...
                    <ul class="navbar-nav float-left">
                        ...
                        ?>
                            <li class="nav-item dropdown ">
                                <a href="<?php echo base_url($valus->page_link) ?>"  class="nav-link fw-bold <?php if (!empty($valus->submenu)) {
                                                                    echo "dropdown-toggle";
                                                                } else {
                                                                    echo "";
                                                                } ?>" id="navbarDropdown" role="button" data-bs-toggle=<?php if (!empty($valus->submenu)) {
                                                                    echo "dropdown";
                                                                } else {
                                                                    echo "";
                                                                } ?> aria-expanded="false">
...
$dyFooter = $this->db->get_where('footer', array('type' => 'about'), 1)->result_array();
$this->db->order_by('id', 'DESC');
$comFooter = $this->db->get_where('footer', array('type' => 'company'), 1)->result_array();
$this->db->order_by('id', 'DESC');
$contFooter = $this->db->get_where('footer', array('type' => 'contact'), 1)->result_array();
?>
